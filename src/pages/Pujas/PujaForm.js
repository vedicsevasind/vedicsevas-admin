...
export default function PujaForm() {
  ...
  const [form, setForm] = useState({
    ...
    benefits: '', whenToPerform: '', duration: '',
    ...
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl p-6 shadow-sm space-y-4">
        {[
          ...
          { name: 'whenToPerform', label: 'When to Perform', type: 'text' },
        ].map(f => (
          <div key={f.name}>
        ...
        {[
          ...
          <div key={f.name}>
            ...
            <textarea name={f.name} value={form[f.name]} onChange={handleChange}
              rows={f.rows} className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm"/>
          </div>
        ))}
        <div className="flex gap-3 pt-2">
          <button type="button" onClick={() => navigate('/pujas')}
